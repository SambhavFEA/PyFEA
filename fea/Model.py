...
class Model:
    ...
    def read_input_2D(self,filename):
        ...
        for l in range(num_of_bc):
            self.boundary_constraints[l] = list(map(float, lines[15 + num_node_coord + num_elem + num_of_mat_prop + l + 1].split(" ")))
        ...
        for m in range(num_of_fc):
            self.forces[m] = list(map(float, lines[19 + num_node_coord + num_elem + num_of_mat_prop + num_of_bc + m + 1].split(" ")))
